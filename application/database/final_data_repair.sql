-- ================================================================
-- FINAL ONLINE SHOP DATA REPAIR / SEED (PostgreSQL)
-- Run AFTER online_shop_schema_v2_postgres.sql.
-- Safe to rerun: no DROP / TRUNCATE / DELETE.
-- ================================================================

BEGIN;

-- Keep the admin login as "admin". Customer usernames are made proper-case
-- because the current screens display i_username directly.
UPDATE mst_user
SET i_username = INITCAP(REPLACE(TRIM(i_username), '_', ' '))
WHERE i_username IS NOT NULL
  AND LOWER(i_username) <> 'admin';

-- Display name column too, when present in the current database.
UPDATE mst_user
SET e_name = INITCAP(REPLACE(TRIM(i_username), '_', ' '));

-- Bank owner and address recipient must match mst_user.
...
-- Add one payment account for every customer who currently has none.
INSERT INTO mst_bank_account (id_user, e_bank_name, i_account_number, e_account_holder, e_keterangan, f_primary)
SELECT
    u.id_user,
    CASE ((u.id_user - 2) % 4)
        WHEN 0 THEN 'BCA'
    ...
    't'
FROM mst_user u
WHERE LOWER(u.i_username) <> 'admin'
  AND NOT EXISTS (SELECT 1 FROM mst_bank_account b WHERE b.id_user = u.id_user);

-- Wuthering Waves names for courier contact persons.
WITH x(id_courier, e_contact_person, i_phone) AS (
    VALUES
...
-- Requested automotive brands.
INSERT INTO mst_brand (e_brand, e_keterangan, f_active)
SELECT v.e_brand, 'Brand kendaraan / otomotif', 't'
FROM (VALUES ('Toyota'),('Kawasaki'),('Honda'),('Mercedes-Benz'),('BMW'),('Ducati')) v(e_brand)
WHERE NOT EXISTS (
    SELECT 1 FROM mst_brand b WHERE LOWER(b.e_brand) = LOWER(v.e_brand)
);

-- Make the product master richer.
...
WHERE NOT EXISTS (
    SELECT 1 FROM mst_product_type t WHERE LOWER(t.e_product_type)=LOWER(v.e_product_type)
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM mst_category c WHERE LOWER(c.e_category)=LOWER(v.e_category)
);
...
-- Add stock-bearing products; existing products stay untouched.
INSERT INTO mst_product (id_category,i_product,e_product,v_price,n_stock,f_active,id_brand,id_unit)
SELECT c.id_category,v.i_product,v.e_product,v.v_price,v.n_stock,'t',b.id_brand,u.id_unit
FROM (VALUES
...
CROSS JOIN LATERAL (SELECT id_unit FROM mst_unit WHERE LOWER(e_unit)='unit' LIMIT 1) u
WHERE NOT EXISTS (SELECT 1 FROM mst_product p WHERE LOWER(p.i_product)=LOWER(v.i_product));

-- Main warehouse and stock detail.
INSERT INTO mst_warehouse (e_warehouse,e_address,f_active)
SELECT 'Gudang Utama','Jakarta','t'
WHERE NOT EXISTS (SELECT 1 FROM mst_warehouse WHERE LOWER(e_warehouse)='gudang utama');
...
WHERE LOWER(w.e_warehouse)='gudang utama'
  AND NOT EXISTS (SELECT 1 FROM mst_warehouse_location wl WHERE wl.id_warehouse=w.id_warehouse);

INSERT INTO trx_stock (id_product,id_warehouse,id_rack,n_qty)
SELECT p.id_product,w.id_warehouse,r.id_rack,p.n_stock
FROM mst_product p
CROSS JOIN LATERAL (
    SELECT id_warehouse FROM mst_warehouse WHERE LOWER(e_warehouse)='gudang utama' ORDER BY id_warehouse LIMIT 1
) w
LEFT JOIN LATERAL (
    SELECT r0.id_rack
    FROM mst_rack r0
    JOIN mst_warehouse_location wl ON wl.id_warehouse_location=r0.id_warehouse_location
    WHERE wl.id_warehouse=w.id_warehouse
    ORDER BY r0.id_rack LIMIT 1
) r ON TRUE
WHERE p.f_active='t'
  AND NOT EXISTS (
      SELECT 1 FROM trx_stock s WHERE s.id_product=p.id_product AND s.id_warehouse=w.id_warehouse
  );

-- Normalize all transaction invoice numbers.
UPDATE trx_transaction SET i_invoice='TMP-FIX-'||id_transaction;
WITH numbered AS (
  SELECT id_transaction, ROW_NUMBER() OVER (ORDER BY id_transaction) AS seq
...
-- Payment account on every transaction points at that customer's primary bank.
UPDATE trx_transaction t
SET id_bank_account=b.id_bank_account
FROM mst_bank_account b
WHERE b.id_user=t.id_user AND b.f_primary='t';
